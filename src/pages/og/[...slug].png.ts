...
const tag = (e: AnyEntry, type: PageMeta["type"]): PageMeta => ({
  type,
  entry: e,
});
...
const route = await OGImageRoute({
  ...
  getImageOptions: (_path, page: PageMeta) => {
    if (page.type === "home") {
      return {
        title:
          "Writing on minds, machines, and philosophy. Making the art that emerges in-between.",
